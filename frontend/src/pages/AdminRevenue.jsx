...
const AdminRevenue = () => {
  ...
  const exportExcel = async () => {
    ...
    // Server-side Excel export so frontend builds don't depend on xlsx being installed
    const response = await api.post('/api/admin/revenue/export.xlsx', dashboardData, {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `STAIRS-Revenue-Report-${new Date().toISOString().split('T')[0]}.xlsx`;
    a.click();
    window.URL.revokeObjectURL(url);
  };
