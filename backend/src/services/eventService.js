...
class EventService {
  ...
  async _processFile(file, eventId, description, uploaderId, uploaderType) {
    try {
      ...
      const uniqueFileName = `event-${eventId}-${timestamp}${extension}`;
      const filePath = path.join(uploadsDir, uniqueFileName);

      // Move file to final location (multer already saved it temporarily)
      ...
      console.log(`✅ File uploaded: ${file.originalname} -> ${uniqueFileName}`);
